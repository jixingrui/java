...
public class AsrDemo implements NlsListener {
	...
	public void startAsr() {
		...
		try {
			String filePath = "./assets/sample.pcm";
		...
		if (fis != null) {
			...
			try {
				...
				req.setAppKey("nls-service"); // appkey请从 "快速开始"
												// 帮助页面的appkey列表中获取
				req.setAsrFormat("pcm"); // 设置语音文件格式为pcm,我们支持16k 16bit
											// 的无头的pcm文件。
				...
				req.authorize("", ""); // 请替换为用户申请到的Access Key ID和Access Key
										// Secret
