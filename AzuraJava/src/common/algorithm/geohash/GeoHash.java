...
public final class GeoHash implements Comparable<GeoHash>, Serializable {
		...
		for (int i = 0; i < binaryString.length(); i++) {
			if (binaryString.charAt(i) == '1') {
				...
				throw new IllegalArgumentException(binaryString
						+ " is not a valid geohash as a binary string");
			}
		}
	...
	public static GeoHash fromLongValue(long hashVal, int significantBits) {
		...
		for (int j = 0; j < significantBits; j++) {
			if (isEvenBit) {
				divideRangeDecode(hash, longitudeRange,
						binaryString.charAt(j) != '0');
			} else {
				divideRangeDecode(hash, latitudeRange,
						binaryString.charAt(j) != '0');
			}
			isEvenBit = !isEvenBit;
		}
	 ...
	 * characters can be achieved.
	 */
	public static String geoHashStringWithCharacterPrecision(double latitude,
			double longitude, int numberOfCharacters) {
		GeoHash hash = withCharacterPrecision(latitude, longitude,
				numberOfCharacters);
	...
	private static void setBoundingBox(GeoHash hash, double[] latitudeRange,
			double[] longitudeRange) {
		hash.boundingBox = new BoundingBox(new WGS84Point(latitudeRange[0],
				longitudeRange[0]), new WGS84Point(latitudeRange[1],
				longitudeRange[1]));
	}
	...
	public GeoHash upper() {
		return withBitPrecision(point.getLatitude(), point.getLongitude(),
				significantBits - 1);
	}

	public GeoHash level(byte significantBits) {
		return withBitPrecision(point.getLatitude(), point.getLongitude(),
				significantBits);
	}
	...
	private static void divideRangeDecode(GeoHash hash, double[] range,
			boolean b) {
		double mid = (range[0] + range[1]) / 2;
		if (b) {
			hash.addOnBitToEnd();
	...
	public GeoHash[] getAdjacent() {
		...
		return new GeoHash[] { northern, northern.getEasternNeighbour(),
				eastern, southern.getEasternNeighbour(), southern,
				southern.getWesternNeighbour(), western,
	...
	public byte[] toBytes() {
		...
		result[0] = significantBits;
		byte[] longBits = Primitives.toByta(bits);
		for (int i = 0; i < 8; i++) {
			result[i + 1] = longBits[i];
		}
		return result;
	}
	...
	protected GeoHash recombineLatLonBitsToHash(long[] latBits, long[] lonBits) {
		...
		for (int i = 0; i < latBits[1] + lonBits[1]; i++) {
			if (isEvenBit) {
				divideRangeDecode(hash, latitudeRange,
						(latBits[0] & FIRST_BIT_FLAGGED) == FIRST_BIT_FLAGGED);
				latBits[0] <<= 1;
			} else {
				divideRangeDecode(hash, longitudeRange,
						(lonBits[0] & FIRST_BIT_FLAGGED) == FIRST_BIT_FLAGGED);
				lonBits[0] <<= 1;
			}
			isEvenBit = !isEvenBit;
		}
	...
	protected long[] getRightAlignedLatitudeBits() {
		...
		long value = extractEverySecondBit(copyOfBits,
				getNumberOfLatLonBits()[0]);
	...
	public String toString() {
		if (significantBits % 5 == 0) {
			return String.format("%s -> %s -> %s", Long.toBinaryString(bits),
					boundingBox, toBase32());
		} else {
			return String.format("%s -> %s, bits: %d",
					Long.toBinaryString(bits), boundingBox, significantBits);
		}
	}
	...
	public String toHexString() {
		return String.format("%s -> %s, bits: %d", Hex.getHex(toBytes()),
				boundingBox, significantBits);
	}
