package azura.banshee.zbase.kdtree;

import azura.banshee.zbase.heaps.BinaryHeap;
import azura.banshee.zbase.heaps.MaxHeap;
import azura.banshee.zbase.heaps.MinHeap;
...
public class KdTree<T> extends KdNode<T> {
	...
	public NearestNeighborIterator<T> getNearestNeighborIterator(
			double[] searchPoint, int maxPointsReturned,
	...
	public MaxHeap<T> findNearestNeighbors(double[] searchPoint,
		...
		while (pendingPaths.size() > 0
				&& (evaluatedPoints.size() < pointsRemaining || (pendingPaths
						.getMinKey() < evaluatedPoints.getMaxKey()))) {
	...
	protected static <T> void nearestNeighborSearchStep(
			MinHeap<KdNode<T>> pendingPaths, MaxHeap<T> evaluatedPoints,
			int desiredPoints, DistanceFunction distanceFunction,
			double[] searchPoint) {
