...
public class MaskStripper {
	...
	public static void strip(File input) throws IOException {
		...
		BufferedImage output = ImageUtil.newImage(
				reader.frameWidth(reader.zMax),
				reader.frameHeight(reader.zMax), new Color(0xff000000, true));

		for (MaskGem gem : mask.color_MaskGem.values()) {
			// gem.drawFill(output);
			gem.drawStrip(output);
		}

		ImageIO.write(output, "png",
				new File(FileUtil.getNoExt(input.getPath()) + "_strip.png"));
