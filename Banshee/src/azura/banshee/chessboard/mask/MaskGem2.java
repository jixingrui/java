...
public class MaskGem2 extends RectanglePlus {
	public static Logger log = Logger.getLogger(MaskGem2.class);
	...
	private MaskGem2(int xLeft, int yDown, ImageReader reader, int color) {
		super(xLeft, yDown, 1, 1);
		this.reader = reader;
		this.color = color;
	}

	static MaskGem2 create(int xLeft, int yDown, ImageReader image, int color) {
		MaskGem2 gem = new MaskGem2(xLeft, yDown, image, color);
	...
	private static void createAccurate(MaskGem2 gem, ImageReader image) {
	...
	public void toShards(Zmask2 mask, int z) {

		HashMap<ZmaskTile2, Shard> t_s = new HashMap<ZmaskTile2, Shard>();

		for (int pos : lbsColor) {
			int xPix = pos % width + x;
			int yPix = pos / width + y;
			ZmaskTile2 tile = mask.getTile(xPix, yPix, z);
			if (tile == null) {
		...
		for (Entry<ZmaskTile2, Shard> e : t_s.entrySet()) {
			ZmaskTile2 tile = e.getKey();
			Shard s = e.getValue();
			s.seal();
	...
	public MaskGem2 shrink() {
		MaskGem2 quarter = new MaskGem2();
