...
public class HardCodeOld {
	protected static Logger log = Logger.getLogger(HardCodeOld.class);
	...
	public HardCodeOld(HardS_CS sc, Helios6<?> db, HardHandlerI user) {
		this.sc = sc;
		this.db = db;
		this.user = user;
	}
	...
	public void doAdd(Batch batch, HardItem hi) {
		...
		// HardItem hi = HardItem.fromNode(newNode);
		// hi.name = name;
		// newNode = hi.getNode();
		...
		batch.link(user.getTagNode(), newNode).link(parentNode, newNode)
				.save(newNode);
	...
	public Batch doDeleteBefore() {
		...
		Batch batch = new Batch().delink(user.getTagNode(), target)
				.delink(selectedMomNode, target).delete(target);
	...
	private void swapId() {
		...
		Batch batch = new Batch().swap(selectedItem.getNode(),
				heldItem.getNode());
...
//		log.info("after swap: " + selectedItem + " " + held);
	...
	public void appendDown() {
		...
		Join join = new Join(indexOfLastRecord + 1, pageSize).addFrom(
				user.getTagNode()).addFrom(p);
	...
	public HardCodeOld getHC(Enum<?> hardE) {
