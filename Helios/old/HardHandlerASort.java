...
public abstract class HardHandlerASort implements HardServerI {
	protected static Logger log = Logger.getLogger(HardHandlerASort.class);
	...
	public HardHandlerASort(boolean isTree, HeliosNode tagNode) {
		this.isTree = isTree;
	...
	public void swapId() {
		...
		Batch batch = new Batch().swap(selected.getNode(false), heldItem.getNode(false)).seal();
	...
	public HeliosNode doAdd(String name, byte[] data) {
		...
		if (downList.isEmpty()) {
			...
			hi.sortValue = downList.peekLast().sortValue + 16;
		} else if (selectedIdx == 0) {
			hi.sortValue = downList.peekFirst().sortValue - 16;
		} else {
			HardItem before = downList.get(selectedIdx - 1);
			HardItem after = downList.get(selectedIdx);
			if (after.sortValue - before.sortValue > 1) {
				hi.sortValue = (before.sortValue + after.sortValue) / 2;
			}
		}
		...
		Batch batch = new Batch().link(tagNode, newNode).link(parentNode, newNode).save(newNode);
	...
	private void deleteNotifyParent() {
		...
		CompletableFuture<Boolean> txn = hub.helios.executeNowCommitFuture(batch);
