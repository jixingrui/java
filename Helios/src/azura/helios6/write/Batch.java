...
public class Batch {
	...
	public List<Hnode> nodeList = new ArrayList<>();
	public Collection<Op> opList = new SortedArrayList<Op>();
	// BitSet writeNodes = new BitSet();
	// BitSet deleteNodes = new BitSet();

	public Batch link(Hnode from, Hnode to) {
		Op op = new Op(OpE.link, this);
		op.one = put(from);
		op.to = put(to);
		opList.add(op);
		return this;
			...
			if (node.getId() > 0) {
				for (Hnode stored : nodeList) {
					if (stored.getId().equals(node.getId()))
						throw new IllegalArgumentException("Batch: different nodes have same id");
				}
			}
			idx = nodeList.size();
			nodeList.add(node);
